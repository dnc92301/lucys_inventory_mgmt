...
function SuccessScreen({ store, orders, onHand, gloveSizes, deliveryDateStr, onNewOrder, submittedBy }: {
  store: string, orders: Record<string, number>, onHand: Record<string, number>,
  gloveSizes: Record<string, number>, deliveryDateStr: string,
  onNewOrder: () => void, submittedBy: string
}) {
...
export default function Home() {
 ...
 //const [email, setEmail] = useState('');
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch('/api/submit-order', {
        ...
        body: JSON.stringify({ store, orders, onHand, gloveSizes, deliveryDateStr, submittedBy: user?.name }),
  ...
  const handleNewOrder = () => { setScreen('form'); setOrders({}); setOnHand({}); setGloveSizes({ XL: 0, L: 0, M: 0 }); setStore(''); setDeliveryDateStr(''); setUser(null); setEmail(''); };
  ...
  if (screen === 'success') return <SuccessScreen store={store} orders={orders} onHand={onHand} gloveSizes={gloveSizes} deliveryDateStr={deliveryDateStr} onNewOrder={handleNewOrder} submittedBy={user?.name || ''} />;
